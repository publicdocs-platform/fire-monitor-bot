...
// Metrics
const monNumLoopsStarted = globalStats.createMeasureInt64(
  "loops_started",
  MeasureUnit.UNIT,
  "The number of main loops started"
);
const monNumLoopsFinished = globalStats.createMeasureInt64(
  "loops_finished",
  MeasureUnit.UNIT,
  "The number of main loops finished (error or success)"
);
const monNumLoopsErrored = globalStats.createMeasureInt64(
  "loops_errored",
  MeasureUnit.UNIT,
  "The number of main loops errored"
);
const monNumLoopsSucceeded = globalStats.createMeasureInt64(
  "loops_success",
  MeasureUnit.UNIT,
  "The number of main loops succeeded"
);
const monLoopLatencySec = globalStats.createMeasureDouble(
  "loop_latency",
  MeasureUnit.SEC,
  "The loop latency in seconds"
);

const monNumFiresProcessed = globalStats.createMeasureInt64(
  "fires_processed",
  MeasureUnit.UNIT,
  "The number of fires processed"
);
const monNumFireMaterialUpdatesProcessed = globalStats.createMeasureInt64(
  "fire_material_updates",
  MeasureUnit.UNIT,
  "The number of fire material updates"
);
const monFireMaterialUpdateProcessLatencySec = globalStats.createMeasureDouble(
  "fire_material_update_latency",
  MeasureUnit.SEC,
  "The latency of a material update"
);
...
exports.builder = {
  ...
  perimAfter: {
    ...
    default: "2017-12-31",
    desc:
      "Only display a update because of a perimeter change after this timestamp"
  },
  twitterAuthPath: {
    string: true,
  ...
  embargoSec: {
    ...
    default: 60 * 10,
    desc:
      "Seconds to wait to post a new tweet (if updated data is found in the interim, the older tweet may be deleted)"
  },
...
exports.handler = argv => {
  ...
  const intensiveProcessingSemaphore = util.namedSemaphore(
    processingSemaphore,
    "computation"
  );
  ...
  const htmlTemplate = pug.compileFile(
    path.join(__dirname, "../templates/render-overview.pug")
  );
  const genHtml = function(entry) {
    return htmlTemplate({ config: config, data: entry, curdir: process.cwd() });
  };

  const perimeterTemplate = pug.compileFile(
    path.join(__dirname, "../templates/render-details.pug")
  );
  const perimeterHtml = function(entry) {
    return perimeterTemplate({
      config: config,
      data: entry,
      curdir: process.cwd()
    });
  ...
  const tweetTemplate = pug.compileFile(
    path.join(__dirname, "../templates/tweet-update.pug")
  );
  ...
  async function internalLoop(isFirstRun, previousDb) {
    ...
    const async = {
      ...
      calfireIncidents: argv.ingestCalfire
        ? calfire.getFires(argv.userAgent)
        : {}
    };
    ...
    mergeNfsaAndGeomacIncidentsIntoDb(
      nfsaIncidents,
      geomacIncidents,
      currentDb
    );
    mergeCalfireIncidentsIntoDb(
      calfireIncidents,
      currentDb,
      argv.mergeDistanceMaxMiles
    );
    ...
    fs.writeFileSync(
      argv.outputdir + "/data/GLOBAL-DIFF-" + globalUpdateId + ".yaml",
      diffsGlobal
    );
    ...
    const currentDbKeysSorted = _.sortBy(
      currentDbKeys,
      i => -currentDb[i].DailyAcres
    );
    // Human summaries
    const updates = [];
    const updateNames = [];
    ...
    fireLoop: for (const key1 of currentDbKeysSorted) {
      ...
      try {
        ...
        let { cur, perimDateTime, inciWeb, perim } = preDiffFireProcess(
          i,
          currentDb,
          previousDb,
          perims
        ...
        for (const oldKey of oldMatchingKeys) {
          if (
            !old ||
            !old.ModifiedOnDateTime ||
            old.ModifiedOnDateTime < previousDb[oldKey].ModifiedOnDateTime
          ) {
            old = _.cloneDeep(previousDb[oldKey]);
            if (old.ModifiedOnDateTime >= cur.ModifiedOnDateTime) {
              logger.debug(
                "  -) Previous record id %s (cur id %s) not updated old %o new %o",
                oldKey,
                i,
                old.ModifiedOnDateTime,
              ...
              if (
                !perimDateTime ||
                (old.PerimDateTime && old.PerimDateTime >= perimDateTime)
              ) {
                logger.debug(
                  "  -) Previous perim ALSO not updated old %o new %o",
                  old.PerimDateTime,
                  perimDateTime,
                  {
                    x: currentDb[i],
        ...
        const updateId =
          ...
          cur.ModifiedOnDateTime +
          "-PER-" +
          (cur.PerimDateTime || "none") +
          "-ID-" +
          i +
          "-NAME-" +
          cur.Name.replace(/[^a-z0-9]/gi, "") +
          "-S-" +
          cur.Source.charAt(0);
        ...
        const updateSummary = `🔥 ${cur.Final_Fire_Name} (${i}) ${cur.Hashtag}
           🕒 ${cur.ModifiedOnDateTime}
        ...
        if (
          !cur.ModifiedOnDateTimeEpoch ||
          cur.ModifiedOnDateTimeEpoch < pruneTime
        ) {
          logger.debug(
            " #! Pruning %s %s -> last mod %s",
            i,
            cur.Name,
            cur.ModifiedOnDateTime,
            { cur: cur, x: currentDb[i] }
          );
        ...
        if (
          !(
            "DailyAcres" in oneDiff ||
            "PercentContained" in oneDiff ||
            "PerimeterData.Acres" in oneDiff
          )
        ) {
          if (!argv.monitorPerims || !("PerimDateTime" in oneDiff)) {
            // Unless acreage, perim, or containment change, we don't report it.x
            logger.info(
              "     -) No perim date diff or not monitored %s %o",
              updateId,
              perimDateTime,
              { diff: oneDiff, updateId: updateId }
            );
            updates.push("⏭️" + updateSummary);
            continue;
          }
          ...
          if (!perimDateTime || perimDateTime <= argv.perimAfter) {
            logger.info(
              "    -) No perim date or old %s %o diff %o",
              updateId,
              perimDateTime,
              { diff: oneDiff, updateId: updateId }
            );
        ...
        if (
          !(
            "PercentContained" in oneDiff || "PerimeterData.Acres" in oneDiff
          ) &&
          old.DailyAcres &&
          cur.DailyAcres &&
          Math.abs(cur.DailyAcres - old.DailyAcres) < 1.1
        ) {
          // May be spurious - due to rounding in GEOMAC vs NFSA.
          logger.info(
            "    -) Insufficient acreage change old %s %o cur %o",
            updateId,
            old.DailyAcres,
            cur.DailyAcres,
            { diff: oneDiff, updateId: updateId }
        ...
        if (fs.existsSync(diffPath)) {
          logger.error(
            "    $$$$ ANOMALY DETECTED - REPEATING UPDATE %s - SKIPPED",
            updateId,
            {
              updateId: updateId,
              diffs: diffs
        ...
        await promisify(intensiveProcessingSemaphore.take).bind(
          intensiveProcessingSemaphore
        )();
        ...
        try {
          didProcess = await internalProcessFire(
            logger,
            updateId,
            inciWeb,
            cur,
          ...
          if (didProcess) {
            ...
            globalStats.record([
              { measure: monNumFireMaterialUpdatesProcessed, value: 1 },
              {
                measure: monFireMaterialUpdateProcessLatencySec,
                value: latency
              }
            ]);
          }
    ...
    fs.writeFileSync(
      argv.db,
      yaml.safeDump(
        { __META: dbMetaInfo, Database: currentDb },
        { skipInvalid: true }
      )
    );
    ...
    async function internalProcessFire(
      parentLogger,
      updateId,
      inciWeb,
      cur,
      ...
      const mainWebpage =
        argv.outputdir + "/img/WEB-INFO-" + updateId + ".html";
      const perimImg = argv.outputdir + "/img/IMG-PERIM-" + updateId + ".jpeg";
      const perimWebpage =
        argv.outputdir + "/img/WEB-PERIM-" + updateId + ".html";
      const mainWebpageUrl =
        "http://localhost:8080/updates/img/WEB-INFO-" + updateId + ".html";
      const perimWebpageUrl =
        "http://localhost:8080/updates/img/WEB-PERIM-" + updateId + ".html";
      if (inciWeb && argv.archiveInciweb) {
        const u =
          "https://web.archive.org/save/https://inciweb.nwcg.gov/incident/" +
          inciWeb +
          "/";
        rp({ uri: u, resolveWithFullResponse: true })
          .then(r => {
            logger.info(
              "   ~~ Archived to web.archive.org: %s",
              r.headers
                ? "https://web.archive.org/" + r.headers["content-location"]
                : "unknown"
            );
          })
          .catch(err => {
      ...
      const meaningfulDistanceMi =
        100 + Math.sqrt(0.0015625 /* mi2 per acre*/ * cur.DailyAcres);
      const cities2 = lat
        ? _.sortBy(
            geocoding.nearestCities(lat, lon, 1000, 500).map(x => {
              if (x.distance < meaningfulDistanceMi) {
                x.useful = true;
                const pointDists = points.map(pp =>
                  geocoding.distance(pp[0], pp[1], x.lon, x.lat)
                );
                const minPointIndex = _.minBy(
                  _.range(0, points.length),
                  idx => pointDists[idx]
                );
                const thePoint = points[minPointIndex];
                x.distance = pointDists[minPointIndex];
                x.bearing = geocoding.bearing(
                  x.lon,
                  x.lat,
                  thePoint[0],
                  thePoint[1]
                ...
                x.directions = geocoding.friendlyDistance(
                  x.distance,
                  "mi",
                  geocoding.compass(x.bearing),
                  x.displayName
                );
                x.weightedPopulation =
                  x.distance < 20
                    ? x.population
                    : x.distance < 50
                      ? 0.5 * x.population
      ...
      const nearPopulation = Math.round(
        cities.reduce((a, b) => a + b.weightedPopulation, 0)
      );
      const allPopulation = cities.reduce((a, b) => a + b.population, 0);
      logger.info(
        "     > Fire %s is near pop. %d (all %d), %d acres, %d staff",
        updateId,
        nearPopulation,
        allPopulation,
        cur.DailyAcres,
        cur.TotalIncidentPersonnel,
        {
          updateId: updateId,
      ...
      const displayFilters = {
        InHawaii:
          cur.state === "HI" ||
          cur.State === "HI" ||
          (_.first(cities) || {}).adminCode === "HI",
        KnownLocationLowPop: lat && lon && nearPopulation <= 1000,
        UnknownLocationSmallSize:
          !lat &&
          !lon &&
          (cur.DailyAcres || 0) < 1.1 &&
          (cur.TotalIncidentPersonnel || 0) < 15,
        FalseAlarmType:
          cur.IncidentTypeCategory === "FA" ||
          cur.incidenttypecategory === "FA",
        FalseAlarmName:
          cur.Fire_Name.toLowerCase().startsWith(" fa fire") ||
          cur.Fire_Name.toLowerCase().startsWith(" fa") ||
          cur.Fire_Name.toLowerCase().startsWith("fa ") ||
          (cur.Fire_Name.toLowerCase().includes("false") &&
            cur.Fire_Name.toLowerCase().includes("alarm")),
        StepUpName:
          cur.Fire_Name.toLowerCase().includes("step-up") ||
          cur.Fire_Name.toLowerCase().includes("step up") ||
          cur.Fire_Name.toLowerCase().includes("stepup"),
        NonStatFRName: cur.Fire_Name.toLowerCase().startsWith("nonstat "),
        // 3 hours with no info, might be stale
        OldEmergingFiresWithoutInfo:
          (cur.NFSAType || "").includes("Emerging") &&
          !cur.DailyAcres &&
          !cur.PercentContained &&
          cur.ModifiedOnDateTimeEpoch - cur.FireDiscoveryDateTimeEpoch >
            1000 * 60 * 60 * 3,
        LACNoData:
          !cur.DailyAcres &&
          !cur.PercentContained &&
          cur.Fire_Name.toLowerCase().substr(0, 4) === "lac-"
      };
      ...
      const delPaths = await del([
        argv.outputdir + "/postqueue/*-" + cur.UniqueFireIdentifier + "-*.yaml"
      ]);
      ...
      const extraTags = [cur.unitMention]
      ...
      fs.writeFileSync(
        argv.outputdir + "/tweets/TWEET-" + updateId + ".txt",
        tweet
      );
      ...
      async function perimAndSaveProcess() {
        ...
        if (detailImg) {
          const perimTemplateData = {
            ...
            mapData: {
              events: events,
              perimSourceLayer: cur.PerimeterData
                ? cur.PerimeterData._Provenance
                  ? cur.PerimeterData._Provenance.SourceLayer
                  : null
                : null,
              qrcode: qrcode
            },
        ...
        const priority = numeral(Math.round(10000000 - invPrio)).format(
          "00000000"
        );
        ...
        const usePerimAcres =
          cur.PerimDateTime &&
          cur.PerimDateTime > cur.ModifiedOnDateTime &&
          cur.PerimeterData.Acres &&
          cur.PerimeterData.Acres > (cur.DailyAcres || 0);
        ...
        const saved = {
          ...
          shortText:
            cur.UniqueFireIdentifier +
            " - Unofficial fire report. See officials for safety info. May be incorrect; disclaimers in images.",
          ...
          threadQuery: argv.twitterThreadQueryPrefix
            ? argv.twitterThreadQueryPrefix + " " + cur.Hashtag
            : null
        };
        if (center) {
  ...
  function preDiffFireProcess(key, x, last, perims) {
    ...
    for (const corrId of cur._CorrelationIds) {
      if (
        corrId in perims &&
        (!perimDateTime || perimDateTime < p.attributes.perimeterdatetime)
      ) {
    ...
    const children = _.values(perims)
      .filter(p => {
        const b =
          (p.attributes.complexname || "").toLowerCase() ===
          (cur.Fire_Name || "").toLowerCase();
        if (b) {
          if (
            !perimDateTime ||
            perimDateTime < p.attributes.perimeterdatetime
    ...
    cur.unitId =
      cur.pooresponsibleunit || cur.UniqueFireIdentifier.split("-")[1];
  ...
  const mainLoop = function(first, last) {
    (async function() {
      ...
      if (!argv.twitterOnly) {
        ...
        const latency = duration[0] + duration[1] / 1000000.0 / 1000.0;
        globalStats.record([
          { measure: monNumLoopsFinished, value: 1 },
          { measure: monLoopLatencySec, value: latency }
        ]);
  ...
  setImmediate(function() {
    mainLoop(
      argv.forceUpdate ? false : persist ? false : true,
      persist ? persist : {}
    );
  });
};

function mergeNfsaAndGeomacIncidentsIntoDb(
  nfsaIncidents,
  geomacIncidents,
  currentDb
) {
  ...
  keys.map(key => {
    currentDb[key] = util.mergedNfsaGeomacFire(
      nfsaIncidents[key],
      geomacIncidents[key]
    );
  });
...
function mergeDupeFires(currentDb, mergeDistanceMax) {
  for (const id of _.keys(currentDb)) {
    ...
    const matchingDbItems = _.filter(currentDb, o => {
      return (
        !_.isEmpty(_.intersection(o._CorrelationIds, item._CorrelationIds)) ||
        (util.fireName(o.Name) === searchName &&
          geocoding.distance(o.Lon, o.Lat, item.Lon, item.Lat) <=
            mergeDistanceMax)
      );
    });
    ...
    if (matchingDbItems.length > 1) {
      ...
      const bestItems = _.sortBy(
        matchingDbItems,
        c => c.ModifiedOnDateTime + c.Source
      ).reverse();
      const bestItemId = bestItems[0].UniqueFireIdentifier;
      const corrIds = _.reduce(
        bestItems,
        (c, d) => _.union(c, d._CorrelationIds),
        []
      );
      ...
      _.map(bestItems, k => {
        if (k.UniqueFireIdentifier !== bestItemId) {
          logger.debug(
            "  >! non-unique fire %s , with best id %s - removing %s",
            id,
            bestItemId,
            k.UniqueFireIdentifier
          );
...
function mergeCalfireIncidentsIntoDb(
  calfireIncidents,
  currentDb,
  mergeDistanceMax
) {
  for (const calfireId of _.keys(calfireIncidents)) {
    ...
    const matchingDbItems = _.filter(currentDb, o => {
      return (
        ...
        util.fireName(o.Name) === searchName &&
        geocoding.distance(o.Lon, o.Lat, calfireItem.Lon, calfireItem.Lat) <=
          mergeDistanceMax
      );
    });
    if (matchingDbItems.length === 0) {
      ...
      currentDb[calfireId] = util.mergedCalfireFire(
        calfireItem,
        matchingDbItems[0]
      );
      ...
      logger.error(
        "Multiple NG incidents match CALFIRE fire = %s",
        searchName,
        {
...
async function getNfsaFires(dataOptions, argv, processNfsaFire) {
  ...
  const filteredLayers = dataSet.layerConfigs.filter(f =>
    requiredLayerNames.includes(f.featureCollection.layerDefinition.name)
  );
  const layerFeatures = filteredLayers.map(x => {
    return x.featureCollection.featureSet.features.map(y => {
      const r = processNfsaFire(
        y,
        "EPSG:" + x.featureCollection.featureSet.spatialReference.latestWkid
      );
  ...
  const data = data0.map(e =>
    Object.assign(e, { _Provenance: _.cloneDeep(prov) })
  );
