...
const twitterDaemon = async function(yamlDir, semaphore, authPath, accountPath, periodMsec) {
  ...
  const retweet = async function(twitterAccount, name, tweetId) {
    ...
    try {
      const retweetData = await twitterAccount.post(retweetUrl, {trim_user: 1});
      console.log(' @@ Posted retweet of ' + tweetId + ' - https://twitter.com/' + name);
    } catch (err) {
      console.log(' !!! Could not statuses/retweet from ' + retweetUrl);
      console.log(err);
    }
  }
  ...
  const postLoop = async function() {
    ...
    try {
      ...
      let replyId = null; let replyTo = null;
      ...
      if (retweetAccount && retwitter && data.data.id_str) {
        ...
        console.log(' @@ Queued retweet of https://twitter.com/' + twitter.name + '/status/' + data.data.id_str + ' from @' + retwitter.name);
