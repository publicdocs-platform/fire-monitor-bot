...
const twitterDaemon = async function(yamlDir, semaphore, authPath, accountPath, periodMsec) {
  ...
  for (const ckey in accountConfig) {
  ...
  const postLoop = async function() {
    ...
    logger.info('@@ Found %d Twitter posts in queue', items.length);
    const p = yamlDir + items[0];
    const plogger = logger.child({file: p});
    try {
      await processTweet(plogger, p, accounts, postMedia, findThread, retweet);
      semaphore.leave();
      taken = false;
...
async function processTweet(logger, p, accounts, postMedia, findThread, retweet) {
  ...
  try {
    ...
    logger.error(' !!! Could not statuses/update from ' + p);
    logger.error(err);
  ...
  logger.info(' @@ Posted new tweet - https://twitter.com/' + twitter.name + '/status/' + data.data.id_str);
