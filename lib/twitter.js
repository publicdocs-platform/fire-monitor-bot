...
const twitterDaemon = async function(yamlDir, semaphore, authPath, accountPath, periodMsec, postToFirestore) {
  ...
  const postLoop = async function() {
    ...
    while (items.length > 0) {
      ...
      try {
        const skipped = await processTweet(plogger, p, accounts, postMedia, findThread, retweet, dbCollection, periodMsec * 150);
        semaphore.leave();
        taken = false;
        if (!skipped) {
...
async function processTweet(logger, p, accounts, postMedia, findThread, retweet, dbCollection, tooOldMsec) {
  const y = await promisify(fs.readFile)(p);
  const item = yaml.safeLoad(y);
  if (!item.embargoUntil || item.embargoUntil < new Date().valueOf() - tooOldMsec) {
    logger.info('@@ Deleting %s for being too old', p);
    await promisify(fs.unlink)(p);
    // Process next one immediately.
    return true;
  }
  if (item.embargoUntil > new Date().valueOf()) {
    // Skipped!
