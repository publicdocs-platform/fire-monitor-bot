...
const getMapBounds = async function(perims, width, height, maxZoom, acres) {
  ...
  const bounds0 = map.determineExtent(14);
  const center0 = [(bounds0[0] + bounds0[2]) / 2, (bounds0[1] + bounds0[3]) / 2];

  const addlMiles = Math.sqrt((acres || 0) / Math.PI * 0.0015625);
  const circleCoords = geocoding.circle(center0[0], center0[1], addlMiles).geometry.coordinates;
  const circle = {
    coords: circleCoords[0],
    color: '#FFFFFFFF',
    fill: '#00000000',
    width: 3,
  };
