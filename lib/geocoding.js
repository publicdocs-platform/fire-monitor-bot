...
exports.compass = function (bearing) {
  ...
  let br = bearing < - offset ? bearing + 360 : bearing;
  // 0 to 360
  let b = br + offset;
  let i = Math.floor(b / (360.0 / names.length));
...
exports.cityDisplayName = function(x) {
  const elems = [x.name, x.adminCode ? (exports.statesByAbbrev[x.adminCode] || x.adminCode) : null, x.country === 'US' ? null : x.country].filter(x => x ? true : false)
  return elems.join(', ');
}
